...
• Проявляй эмпатию к ситуации пользователя"""
    ...
    def _format_consultation_response(
        ...
        formatted_response = (
            ...
            f"{response}\n\n"
            f"💬 **Есть дополнительные вопросы?** Используйте кнопки ниже для продолжения консультации."
        )
    ...
    def _get_error_response(self, category: str) -> str:
        ...
        return (
            ...
            "• Нажать кнопку 'Продолжить консультацию' для повтора\n"
    ...
    async def analyze_document(
        ...
        try:
            ...
            formatted_analysis = self._format_document_analysis_response(
                analysis_content, analysis_type
            )
...
• Указывай номера статей и пунктов при ссылках на законы"""
        ...
        analysis_prompts = {
            'document_summary': """
СПЕЦИАЛИЗАЦИЯ: КРАТКОЕ ОПИСАНИЕ ДОКУМЕНТА

Твоя задача - дать краткое и понятное описание документа.

ИНСТРУКЦИИ:
• Определи тип документа (договор, заявление, справка, письмо и т.д.)
• Укажи основное назначение документа
• Перечисли ключевых участников/стороны
• Кратко опиши основное содержание (2-3 предложения)
• Отметь важные даты, суммы, сроки (если есть)

ФОРМАТ ОТВЕТА:
📋 **Тип документа:** [название типа]
🎯 **Назначение:** [для чего создан документ]
👥 **Участники:** [кто участвует]
📝 **Содержание:** [краткое описание в 2-3 предложениях]
📅 **Ключевые данные:** [даты, суммы, сроки - если есть]

Отвечай кратко, без лишних подробностей. Максимум 200 слов.
""",
            'law_compliance': """
...
5. **Рекомендации по дальнейшим действиям**"""
        ...
        return base_prompt + analysis_prompts.get(analysis_type, analysis_prompts['law_compliance'])
...
Используй структурированный формат ответа и будь максимально конкретным."""
    ...
    def _format_document_analysis_response(self, analysis_content: str, analysis_type: str) -> str:
        """Форматирует ответ анализа документа"""
        
        # Заголовок с типом анализа
        analysis_names = {
            'document_summary': '📋 Краткое описание документа',
            'law_compliance': '⚖️ Проверка соответствия закону',
            'error_detection': '🔍 Поиск ошибок',
            'risk_assessment': '⚠️ Оценка правовых рисков',
            'improvement_suggestions': '💡 Рекомендации по улучшению',
            'business_correspondence': '📧 Анализ деловой переписки'
        }
        
        analysis_name = analysis_names.get(analysis_type, 'Анализ документа')
        header = f"**{analysis_name}**\n\n"
        
        # Для краткого описания не добавляем footer с предупреждением
        if analysis_type == 'document_summary':
            return header + analysis_content
        
        # Для остальных типов анализа добавляем footer
        footer = f"""
